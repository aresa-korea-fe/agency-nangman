...
export default function Home() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // 화면 중간보다 위에 있을 경우
      if (chapterTopPosition < (window.innerHeight / 4) * 1) {
        if (shouldDisplay) {
          setOpacity(0); // 먼저 opacity를 0으로
          setTimeout(() => setShouldDisplay(false), 200); // 0.5초 후에 display를 none으로
        }
      } else {
        if (!shouldDisplay) {
          setShouldDisplay(true); // 먼저 display를 block으로
  ...
  return (
    <main className="h-screen bg-[#040002] text-white">
      ...
      <div className='relative flex justify-center items-center h-screen tracking-normal'>
        <div className='font-bold text-9xl leading-tight text-center'>
          <span className='font-medium'>NangManm</span>
          <br />
          <span className='font-tenada'>Just For You</span>
        </div>
